...
namespace velapex {
namespace perfect_hash_map {

// encode 无效时返回的 key，可与 encode() 返回值比较以判断是否合法
constexpr size_t kInvalidKey = std::numeric_limits<size_t>::max();
...
public:
	...
	// 将代码编码为唯一整数 key，无效返回 kInvalidKey；无需实例即可调用
	...
	// 快速 encode，不做检查，假设 code 一定合法；无需实例即可调用
...
public:
	...
	// 将 6 位数字代码编码为唯一整数 key，无效返回 kInvalidKey；无需实例即可调用
	...
	// 快速 encode，不做检查，假设 key 一定合法；无需实例即可调用
