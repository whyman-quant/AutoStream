...
struct Contract {
	...
	std::unordered_map<int, std::vector<void *> > *quote_hist;
	/* big lock to this instance */
	std::mutex mutex_;
};
...
// my_factor_v2 含 GNU 柔性数组成员（value[0]）；作为另一结构体成员嵌入时 -pedantic 会告警。
// 此处单独屏蔽，不改变字段布局或与协议侧对齐方式。
#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wpedantic"
#endif
typedef struct bulk_factor
{
	char reserve[24];
	my_factor_v2 data;
}bulk_factor_t;
...
typedef struct bulk_factor_datas
{
	int rows;
	int columns;
	int item_size;
