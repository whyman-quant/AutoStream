...
uint32_t my_crc32_value(const char* str_key, size_t len)
{
    uint32_t res = 0 ^ 0xffffffff;
    res = _mm_crc32_u32(res, (uint32_t)(*(uint32_t*)&str_key[0]));
    size_t i = 4;


    while (i < len)
    {   
        res = _mm_crc32_u8(res, (uint8_t)(*(uint8_t*)&str_key[i++]));
    }   
    return res;
}
...
long unix_ns_to_long_us(long unix_ns)
{
    struct tm ts;
    time_t seconds = unix_ns / 1000000;
    ts = *localtime(&seconds);
    return (long)(ts.tm_hour * 10000 + ts.tm_min * 100 + ts.tm_sec) * 1000000 + unix_ns % 1000000;
}
