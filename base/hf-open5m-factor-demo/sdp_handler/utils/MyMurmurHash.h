#ifndef __MYMURMURHASH_H__
#define __MYMURMURHASH_H__
...
*          4. Don't point to item in MyMurmurHash, every reallocate may make pointers invalid.
*
* @author  zengzhiyong
* @date    20200616
*
...
public:
	MyMurMurHash(size_t size = MAX_HASH_SIZE) {
...
private:
	size_t get_hash_value(const char* key, bool expand_flag = false) {
		if (expand_flag == true)
			return my_MurmurHash(key, strlen(key)) & (2 * m_hash_size - 1);
		return my_MurmurHash(key, strlen(key)) & (m_hash_size - 1);
	}
