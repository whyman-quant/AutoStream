#ifndef __MYCRCHASH_H__
#define __MYCRCHASH_H__
...
*          2. Recommend to use get_next_free_node() to avoid copy object twice
*          4. Don't point to item in MyCRCHash, every reallocate may make pointers invalid.
*
* @author  zengzhiyong
* @date    20200616
*
...
public:
	MyCRCHash(size_t capacity = MAX_HASH_SIZE) {
		m_hash_size = ROUND_UP(capacity);
...
private:
	size_t get_hash_value(const char* key, bool expand_flag = false) {
		if (expand_flag == true)
			return my_crc32_value(key,strlen(key)) & (2 * m_hash_size - 1);
		return my_crc32_value(key, strlen(key)) & (m_hash_size - 1);
	}
