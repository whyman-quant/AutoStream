...
OrderHash::OrderHash(){
	...
	int mem_bytes = MAX_ORDER_SIZE * sizeof(Order) +
		HASH_ORDER_SIZE * sizeof(hash_bucket);

	void * all_mem = calloc(1, mem_bytes);
	if (all_mem == NULL) {
		...
		// 故用 "%s" 与第二实参传入正文。
		LOG_LN("%s", "init_hash_table malloc memory fail.");
		return;
	}
...
OrderHash::update_order(int index, Contract *a_contr,
	double price, int order_size, DIRECTION side,
	uint64_t sig_id, OPEN_CLOSE openclose)
{
	...
	if (item == NULL){
		...
		// 展开到 printf 式的 check_log_ln 会触发 ISO C99 可变参数相关告警，
		// 故用 "%s" 与第二实参传入正文。
		LOG_LN("%s", "OrderHash Can't get free order.");
		return NULL;
	}

	set_parameter(item, a_contr, price, order_size, side, sig_id, openclose);
	...
	m_use_count++;
	add_pending_size(item, order_size);
	return item;
}
