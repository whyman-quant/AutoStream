...
int main(){
 const std::vector<std::string> names={"impact_efficiency_signed_price_impact_w16","impact_efficiency_signed_price_impact_w32","impact_efficiency_signed_price_impact_w64","impact_efficiency_signed_price_impact_w128","impact_efficiency_mid_price_response_w16","impact_efficiency_mid_price_response_w32","impact_efficiency_mid_price_response_w64","impact_efficiency_mid_price_response_w128","impact_efficiency_ofi_response_w16","impact_efficiency_ofi_response_w32","impact_efficiency_absorption_divergence_w16","impact_efficiency_absorption_divergence_w32","impact_efficiency_l4g1_param_a","impact_efficiency_l4g1_param_b","impact_efficiency_l4g1_mechanism_a","impact_efficiency_l4g1_mechanism_b"};
 if(factors::impact_efficiency::GetMetadata().factor_names!=names)return 1;
 factors::comm::FactorEntryConfig c; factors::impact_efficiency::FactorEntry buy("000001",factors::impact_efficiency::GetMetadata(),c); buy.AddTrans(T('B',10000,100)); buy.AddTrans(T('B',10100,100)); if(!(V(buy)>0.0))return 1; auto values=Vs(buy); if(values.size()!=16)return 1; for(double x:values)if(!std::isfinite(x))return 1;
 auto opening_mask=buy.GetReadinessMask(92700000); if(opening_mask.size()!=16)return 1; for(size_t i=0;i<10;++i)if(opening_mask[i])return 1; if(!opening_mask[10]||!opening_mask[11]||opening_mask[12]||opening_mask[13]||opening_mask[14]||!opening_mask[15])return 1;
 factors::FactorSetColumnLayout layout; layout["impact_efficiency"] = factors::FactorSetLayout(0, 16);
 factors::FactorEntryRowWriter writer("000001", c, {"impact_efficiency"}, layout, 16);
